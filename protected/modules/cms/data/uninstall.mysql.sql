...
DELETE FROM `<DB_PREFIX>frontend_menu_translations` WHERE `menu_id` IN (SELECT id FROM `<DB_PREFIX>backend_menus` WHERE `module_code` = 'cms');
DELETE FROM `<DB_PREFIX>frontend_menus` WHERE `module_code` = 'cms';
