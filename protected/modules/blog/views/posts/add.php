<?php
	Website::setMetaTags(array('title'=>A::t('blog', 'Add Post')));

    $this->_pageTitle = A::t('blog', 'Posts Management').' - '.A::t('blog', 'Add Post').' | '.CConfig::get('name');
...
<div class="bloc">
	...
	<div class="sub-title"><?= A::t('blog', 'Add Post'); ?></div>
    <div class="content">
        <?php
			echo $actionMessage;          
			// Open form
			$formName = 'frmPostAdd';
			echo CHtml::openForm('posts/insert', 'post', array('name'=>$formName, 'class'=>'widget-cformview', 'autoGenerateId'=>true));
        ?>
		...
		<div class="left-side" id="left-editpost">
			...
			<div class="row">
				...
				<input id="post_header" type="text" value="<?= CHtml::encode($postHeader); ?>" name="post_header" class="large" maxlength="255">
			</div>
			<div class="row">
				<label for="post_text"><?= A::t('blog', 'Post Text'); ?>: <span class="required">*</span></label>
				<div style="float:left">
					<textarea id="post_text" name="post_text" class="full" maxlength="10000"><?= $postText; ?></textarea>
				</div>
	    ...
	    <div class="central-part" id="right-editpost">
			<div class="row">
				...
				<span id="publish_status">
					<input id="publish_status_0" value="0" <?= ($publishStatus == 0 ? 'checked="checked"' : ''); ?> type="radio" name="publish_status"> <label for="publish_status_0"><?= A::t('blog', 'No'); ?></label>
					<input id="publish_status_1" value="1" <?= ($publishStatus == 1 ? 'checked="checked"' : ''); ?> type="radio" name="publish_status"> <label for="publish_status_1"><?= A::t('blog', 'Yes'); ?></label>
				</span>
			</div>
            <? if(!empty($shortCodes)): ?>
			<div class="row">
			...
			<div class="buttons-wrapper">
				<input class="button white" onclick="$(location).attr('href','posts/manage');" value="<?= A::t('blog', 'Cancel'); ?>" type="button">
				<input value="<?= A::t('blog', 'Create'); ?>" type="submit">
			</div>
		</div>
