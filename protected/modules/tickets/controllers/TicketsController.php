...
class TicketsController extends CController
{
    ...
    public function __construct()
    {
		...
		$configModule = CLoader::config('tickets', 'main', true);
		$this->_ticketMembers = isset($configModule['ticketMembers']['loginUrl']) ? $configModule['ticketMembers']['loginUrl'] : array();
		$this->_ticketMembersRole = isset($configModule['ticketMembers']['memberRole']) ? $configModule['ticketMembers']['memberRole'] : array();
    }
    ...
    {
        Website::prepareBackendAction('manage', 'tickets', 'tickets/manage');
    ...
    {
        Website::prepareBackendAction('editTicket', 'tickets', 'tickets/manage/');
    ...
    {
        Website::prepareBackendAction('deleteTicket', 'tickets', 'tickets/manage');
        $model = Tickets::model()->findByPk($id);
	...
	public function userManageTicketsAction($type = '')
	{
		...
		if(!CAuth::getLoggedId() || CAuth::getLoggedRole() != $this->_ticketMembersRole){

			if(CAuth::isLoggedIn() && CAuth::getLoggedRole() != $this->_ticketMembersRole){
				$this->redirect('tickets/manage');
			}
			
			$alert = A::t('tickets', 'Please log in to view this page!', array('{MEMBERS_LOGIN_URL}'=>$this->_ticketMembers));
			$alertType = 'warning';
			if(!empty($alert)){
