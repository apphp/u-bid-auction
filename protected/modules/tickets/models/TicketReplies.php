...
class TicketReplies extends CActiveRecord
{
    ...
    public function __construct()
    {
		...
		$this->_tableUserAccounts = isset($configModule['ticketMembers']['dbTable']) ? $configModule['ticketMembers']['dbTable'] : array();
    }
	...
	protected function _afterSave($pk = '')
	{
		...
		if($this->isNewRecord()){
			...
			if($modelReplies->account_role == 'admin'){
			...
			}elseif($modelReplies->account_role == 'member'){
				...
				$fullname = $modelTicket->first_name.' '.$modelTicket->last_name;
				$sendTicketsNeedsReplyByAdmin = ModulesSettings::model()->param('tickets', 'email_on_user_reply');
	...
	{
		return array('CONCAT(first_name, " ", last_name)' => 'fullname_admin');
	}
