<?php
/**
 * This file implements functionality classic auction
 *
 * Usage:
 *
 * CLoader::library('libauction/AuctionType.php');
 * $classic = AuctionType::init(1);
...
class ClassicAuction extends AuctionGateway
{
	/** @str */
	const NL = "\n";
		...
		if (empty($memberId)) {
			...
			if ($auction) {
				...
				if ($checkActiveAuction['active']) {
					...
					if (($auction->current_bid + $auction->size_bid) != $nextBid) {
						$status = false;
						...
						$bidHistory->size_bid = $nextBid;
						$bidHistory->created_at = date('Y-m-d H:i:s');
						
						$forceSave = APPHP_MODE == 'demo' ? true : false;
						if ($bidHistory->save(null, $forceSave)) {
							// Update auction info
							$updateAuction = Auctions::model()->update(
								$auction->id,
								array('current_bid' => $nextBid),
								true,
								$forceSave
							);
							...
							if ($updateAuction) {
								...
								$updateData = '{
                                    ...
                                    "currentBid": "' . (CCurrency::format(CHtml::encode($nextBid))) . '",
                                    "nextStep": "' . (CCurrency::format(CHtml::encode($nextBid + $auction->size_bid))) . '",
                                    "nextBid": "' . (CHtml::encode($nextBid + $auction->size_bid)) . '",
				...
				if (!empty($auctionId)) {
					...
					if ($auction) {
						...
						if ($activeAuction['active']) {
							...
							$nextBidAuction = $auction->current_bid + $auction->size_bid;
	...
	{
		A::app()->getClientScript()->registerScriptFile('assets/modules/auctions/js/classic.js', 2);
	}
